...
| Per-device alert fix | **done** — on this branch, see Git below |
...
## Git — read this before assuming what is where

This work lives on branch **`claude/kitchen-screen`**, cut from `origin/main`
and developed in a git worktree at `~/Projects/e_ink_screen-kitchen` so the
main checkout stayed free for a concurrent session. **PR #13 (bedtime alert)
is already merged**; if local `main` looks behind, it is just stale.

**A separate, concurrent session owns the CTA transit screen** and shares three
files with this work:

- `server/local-dashboard-server.js`
- `server/config.js`
- `server/package.json`

That session committed *everything* untracked in the main checkout — transit
**and** kitchen — into a single local commit (`b354b5f`, unpushed at the time
of writing). This branch deliberately carries **only** the kitchen half of
those three files, so expect an overlap when both land: the kitchen hunks may
appear twice, and the resolution is to keep one copy, not to re-derive them.
Do not stage, revert, or "tidy" transit files (`server/transit-service.js`,
`transit-stops.js`, `transit-service.test.js`, `server/fixtures/`,
`trmnl-preview.py`, `TRANSIT_SCREEN_PLAN.md`) from here.

**Kitchen-owned files (nothing else touches them):**
...
**Kitchen edits inside the shared files** (small, easy to re-apply if a merge
mangles them): the `DeviceAlertRegistry` import + construction, `device`-aware
